...
public class RenderPlanetUIEntity extends Render<EntityUIPlanet> implements IRenderFactory<EntityUIPlanet> {
	...
	public void doRender(EntityUIPlanet entity, double x, double y, double z,
		...
		if(hitObj != null && hitObj.entityHit == entity) {
			...
			RenderHelper.renderTag(Minecraft.getMinecraft().thePlayer.getDistanceSq(hitObj.hitVec.xCoord, hitObj.hitVec.yCoord, hitObj.hitVec.zCoord), properties.getName(), 0, .6, 0, 5);
	...
	private void renderMassIndicator(VertexBuffer buffer, float percent) {
		buffer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
	...
	private void renderATMIndicator(VertexBuffer buffer, float percent) {
		buffer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
	...
	private void renderTemperatureIndicator(VertexBuffer buffer, float percent) {
		buffer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
