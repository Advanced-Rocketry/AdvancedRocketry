...
public class EntityRocket extends EntityRocketBase implements INetworkEntity, IModularInventory, IProgressBar, IButtonInventory, ISelectionNotify, IPlanetDefiner {
	...
	public void launch() {
		...
		stats.orbitHeight = 0;
		if (orbit) {
			stats.orbitHeight += ARConfiguration.getCurrentConfig().orbit;
		}
		if (stationClearance) {
			stats.orbitHeight += ARConfiguration.getCurrentConfig().stationClearanceHeight;
		}
		if (TBI) {
			//TBI Multiplier
			...
			stats.orbitHeight += (int)(Math.pow(bodyDistanceMultiplier, 0.5d) * ARConfiguration.getCurrentConfig().transBodyInjection);
		}
		if (warp) {
			stats.orbitHeight += (10 * ARConfiguration.getCurrentConfig().transBodyInjection);
		}
