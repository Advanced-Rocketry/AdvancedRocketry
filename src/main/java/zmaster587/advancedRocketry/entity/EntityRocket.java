...
public class EntityRocket extends EntityRocketBase implements INetworkEntity, IModularInventory, IProgressBar, IButtonInventory, ISelectionNotify, IPlanetDefiner {
	...
	/**
	 * Sets the the status of orbit of the rocket and updates the datawatcher
		...
		if(getInSpaceFlight())
		{
			...
			if(burningFuel || descentPhase) {
				...
				if(!world.isRemote && !descentPhase) {
					setFuelAmountBipropellant(getFuelAmountBipropellant() - stats.getFuelRate(FuelType.LIQUID_BIPROPELLANT));
					setFuelAmountOxidizer(getFuelAmountOxidizer() - stats.getFuelRate(FuelType.LIQUID_OXIDIZER));
					setFuelAmountMonoproellant(getFuelAmountMonopropellant() - stats.getFuelRate(FuelType.LIQUID_MONOPROPELLANT));
