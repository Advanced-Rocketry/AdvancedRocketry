				...
				for(int z = 15; z >= 0; z--) {
					for (int y = 254; y >= 0; y--) {
						...
						if (!isCraterIgnoredBlock(chunkPrimerIn.getBlockState(x, y, z).getBlock())) {
							...
							int blockRadius = (baseRadius*baseRadius - (((xCoord * 16) + x) * ((xCoord * 16) + x) + ((zCoord * 16) + z) * ((zCoord * 16) + z))) / (radius * 2);
							...
							if (blockRadius >= 0 && (y - blockRadius > 0)) {
								...
								chunkPrimerIn.setBlockState(x, y - blockRadius, z, this.getBlockToPlace(world, chunkX, chunkZ, ores));
								chunkPrimerIn.setBlockState(x, y - 1 - blockRadius, z, this.getBlockToPlace(world, chunkX, chunkZ, ores));
							}
							break;
						}
					}
