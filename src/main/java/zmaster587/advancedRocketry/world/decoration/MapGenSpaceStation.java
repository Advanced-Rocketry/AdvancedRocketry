...
public class MapGenSpaceStation {
	...
	public static void generateStation(World world, int blockX, int blockY, int blockZ) {
		...
		for(Direction dir : Direction.HORIZONTALS) {
			world.setBlockState(pos.offset(dir), Blocks.PISTON.getDefaultState().with(BlockPistonBase.FACING, dir), 6);
			generateArm(world, pos, dir);
		}

	}

	private static void generateArm(World world, BlockPos pos, Direction direction) {
		for(int i = 0; i < 8; i++) {
