...
public class BlockMultiblockMachine extends RotatableBlock {

	private IIcon sides_active, front_active, rear_active, bottom_active,top_active;
	private Class<? extends TileEntity> tileClass;
	private String textureSideName, textureFrontName, textureTopName, textureBottomName, textureRearName;
	private String textureSideName_active, textureFrontName_active, textureTopName_active, textureBottomName_active, textureRearName_active;

	private int guiId;
	
	public BlockMultiblockMachine(Class<? extends TileEntity> tileClass, int guiId) {
		super(Material.circuits);
		this.tileClass = tileClass;
		this.guiId = guiId;
		this.setCreativeTab(CreativeTabs.tabTransport).setHardness(1F).setResistance(3F).setBlockTextureName("Advancedrocketry:machineGeneric");
	}
	...
	public void onBlockPreDestroy(World world, int x,
		...
		TileMultiBlockMachine tile = (TileMultiBlockMachine)world.getTileEntity(x, y, z);
		tile.deconstructMultiBlock(world, x, y, z, false);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9)
	{
		...
		return tile.attemptCompleteStructure();
	}
